...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.loading_flag = ctk.BooleanVar(value=False)

        # Blur overlay
        self.blur_frame = ctk.CTkFrame(self, fg_color="black", corner_radius=0)
        self.blur_frame.place(relx=0.5, rely=0.5, anchor=ctk.CENTER, relwidth=1, relheight=1)
        self.blur_frame.lower()

        # Loading spinner
        self.spinner_label = ctk.CTkLabel(self.blur_frame, text="", font=ctk.CTkFont(MAIN_FONT, 40))
        self.spinner_label.place(relx=0.5, rely=0.5, anchor=ctk.CENTER)
    ...
    def display_thumbnails(self, folder_path):
        ...
        if not image_paths:
            messagebox.showerror("Error", "The selected folder does not contain any images.")
            return
    ...
    def run_similarity(self):
        ...
        # Show loading screen and blur overlay with spinner
        self.loading_flag.set(True)
        self.loading_label.place(relx=0.5, rely=0.45, anchor=ctk.CENTER)
        self.loading_progress.place(relx=0.5, rely=0.55, anchor=ctk.CENTER)
        self.loading_progress.start()
        self.blur_frame.lift()
        self.update_idletasks()
        self.start_spinner()

        # Run the similarity search in a separate thread to keep the UI responsive
        threading.Thread(target=self._run_similarity_thread, args=(query, folder_path)).start()

    def _run_similarity_thread(self, query, folder_path):
        image_paths = self.load_images(folder_path)
        if not image_paths:
            self.after(0, self._hide_loading)
            self.after(0, lambda: messagebox.showerror("Error", "The selected folder does not contain any images."))
            return
        ...
        # Schedule the image display logic to run on the main thread
        self.after(0, self._hide_loading)
        self.after(0, lambda: self.display_best_image(best_image_path, query))

    def _hide_loading(self):
        self.loading_flag.set(False)
        self.loading_progress.stop()
        self.loading_progress.place_forget()
    ...
    def update_spinner(self):
        if not self.spinner_running:
            return
        spinner_chars = ["|", "/", "-", "\\"]
        current_char = spinner_chars[int(time.time() * 4) % len(spinner_chars)]
        self.spinner_label.configure(text=current_char)
    ...
    def display_best_image(self, image_path, query):
        ...
        try:
            ...
            new_width = 100  # Set your desired width here
            img.thumbnail((new_width, new_width))  # Resize image for the display area
            img_tk = ImageTk.PhotoImage(img)
